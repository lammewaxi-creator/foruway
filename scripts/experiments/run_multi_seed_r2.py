#!/usr/bin/env python3
"""
多种子主实验：在 R2 完整数据集上运行多种子实验
生成表7（算法对比）和表8（统计检验）所需数据
"""
import os
import sys
import json
...
print("=" * 80)
print(f"Multi-seed experiment / R2 full dataset / pop={POP} / gen={GEN} / {len(SEEDS)} seeds")
print("=" * 80)
...
out = "results/multi_seed/multi_seed_r2_results.json"
with open(out, "w", encoding="utf-8") as f:
    json.dump({
        "config": {"jobs_file": JOBS_FILE, "pop_size": POP, "n_gen": GEN, "n_seeds": len(SEEDS), "seeds": SEEDS},
